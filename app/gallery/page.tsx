        ...
        <div className="columns-1 md:columns-2 lg:columns-3 gap-8 space-y-8">
          {visuals.map((img, idx) => (
            <a key={idx} href={img.url} target={img.url.startsWith("http") ? "_blank" : "_self"} rel="noopener noreferrer" className={`block w-full ${img.aspect} bg-surface border border-border flex items-end justify-center relative overflow-hidden group shadow-cinematic break-inside-avoid shadow-sm rounded-2xl`}>
              ...
              <span className="font-label text-[10px] uppercase tracking-[0.3em] text-background bg-accent px-4 py-2 mb-8 relative z-10 md:translate-y-4 md:opacity-0 md:group-hover:translate-y-0 md:group-hover:opacity-100 transition-all duration-500 font-bold">{img.tag}</span>
            </a>
          ))}
        </div>
      </div>
